...
const quote: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
    fastify.post<{ Body: QuoteHeaders }>("/quote", async (request, reply) => {
        try {
            const avatar = await loadSource<Image>(request.body.image_url).catch(() => reply.badRequest("invalid buffer provided"))
            const fakeGradient = await loadSource<Image>("https://imgur.com/29uSOP0.png")
            const quoteFont = await loadFont("https://github.com/zeyrbot/assets/raw/main/fonts/IBMPlexSerif-MediumItalic.ttf")
            ...
            const quoteText = await Image.renderText(quoteFont, 48, `“${request.body.quote}”`, undefined, quoteTextLayout)
            const userText = await Image.renderText(quoteFont, 20, `— ${request.body.user}`)
