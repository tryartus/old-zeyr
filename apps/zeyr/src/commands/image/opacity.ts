...
import { Declare, FailFunction, OKFunction, Options, SubCommand, createOption } from "@potoland/core";
...
export const imageOpacityOptions = {
    ...
    opacity: createOption({
        ...
        value(value: number, ok: OKFunction<number>, fail: FailFunction) {
            if (value > 1 || value < 0) fail(Error("value should be between 1 and 0"))
            ok(value);
        },
    }),
}
