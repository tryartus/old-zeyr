...
export default createMiddleware<void>(async (middleware) => {
    ...
    if (ratelimit.limited) {
        return middleware.stop(Error(`you are being ratelimited, please wait ${seconds.fromMilliseconds(ratelimit.remainingTime)}ms until you can run this command again`))
    }
